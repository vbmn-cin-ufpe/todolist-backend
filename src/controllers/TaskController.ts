...
export class TaskController {
  ...
  async editTask(req: Request, res: Response): Promise<void> {
    try {
      ...
      const taskDto = plainToInstance(CreateTaskDTO, updates);
      await validateOrReject(taskDto);
      const task = await this.taskService.editTask(taskId, taskDto);
  ...
  async deleteTask(req: Request, res: Response): Promise<void> {
    try {
      ...
      res.status(400).json({ error: `Error to delete task` });
    }
  }
